...
model Exchanger "simple generic model"
	...
	initial equation
		...
		deltaT_des[i]=stprops("T","P",1e5,"H",855004)-stprops("T","P",25e6,"H", 1e6);
			state_CO2_des[i] =stprops("H","T",T_CO2_des[i],"P",p_in_CO2_des); 
			state_HTF_des[i] = stprops("H","T",T_HTF_des[i],"P",p_in_HTF_des);
		end for;
	...
	for i in 1:N_exch loop
deltaT[i] = if m_sup then stprops("T","P",HTF_port_a.p,"H",h_HTF[i])-stprops("T","P",CO2_port_a.p,"H",h_CO2[i] ) else deltaT_des[i];
		state_CO2[i] =stprops("T","P",CO2_port_a.p,"H",h_CO2[i] );
		state_HTF[i] = stprops("T","P",HTF_port_a.p,"H",h_HTF[i]);
	end for;
