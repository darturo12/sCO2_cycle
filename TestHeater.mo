...
model TestHeater
  ...
  Modelica.Fluid.Sources.FixedBoundary sourceHot(redeclare package Medium = MediumPB, T = T_htf_in_des, nPorts = 1, p = p_htf, use_T = true, use_p = true) annotation(
  ...
  Modelica.Fluid.Sources.MassFlowSource_h sinkHot(redeclare package Medium = MediumPB, m_flow = -0.1, nPorts = 1, use_m_flow_in = true) annotation(
    Placement(visible = true, transformation(origin = {-60, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Fluid.Sources.FixedBoundary sourceCold(redeclare package Medium = MediumPB, T = T_comp_out_des, nPorts = 1, p = p_turb_in_des, use_T = true, use_p = true) annotation(
    Placement(visible = true, transformation(origin = {-60, -20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Fluid.Sources.MassFlowSource_h sinkCold(redeclare package Medium = MediumPB, m_flow = -0.1, nPorts = 1, use_m_flow_in = true) annotation(
    Placement(visible = true, transformation(origin = {60, -20}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  Modelica.Blocks.Sources.RealExpression m_cold(y = m_flow_pc_des) annotation(
    Placement(visible = true, transformation(origin = {100, -12.5}, extent = {{30, -10}, {-10, 10}}, rotation = 0)));
  sCO2_cycle.Recuperador recuperador(fixed_m_flow = false) annotation(
    Placement(visible = true, transformation(origin = {1, 5}, extent = {{-21, -21}, {21, 21}}, rotation = 0)));
equation
  ...
  connect(sinkHot.ports[1], recuperador.port_a_out) annotation(
    Line(points = {{-50, 20}, {-20, 20}, {-20, 16}, {-20, 16}}, color = {0, 127, 255}));
  connect(recuperador.port_a_in, sourceHot.ports[1]) annotation(
    Line(points = {{22, 16}, {42, 16}, {42, 20}, {50, 20}, {50, 20}}, color = {0, 127, 255}));
  connect(sourceCold.ports[1], recuperador.port_b_in) annotation(
    Line(points = {{-50, -20}, {-26, -20}, {-26, -6}, {-20, -6}, {-20, -6}}, color = {0, 127, 255}));
  connect(recuperador.port_b_out, sinkCold.ports[1]) annotation(
    Line(points = {{22, -6}, {30, -6}, {30, -20}, {50, -20}, {50, -20}}));
  annotation(
