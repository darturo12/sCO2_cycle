...
import Modelica.SIunits.Conversions.from_degC;
  ...
  Modelica.Fluid.Sources.MassFlowSource_T boundary(redeclare package Medium = MediumPB, m_flow = -85.8, nPorts = 1, use_m_flow_in = false) annotation(
    Placement(visible = true, transformation(origin = {48, -8}, extent = {{-8, -8}, {8, 8}}, rotation = 0)));
  Modelica.Fluid.Sources.Boundary_pT Sourc(redeclare package Medium = MediumPB, T = 800 + 273.15, nPorts = 1, p = 25e6, use_T_in = false, use_p_in = false) annotation(
    Placement(visible = true, transformation(origin = {45, 71}, extent = {{-7, -7}, {7, 7}}, rotation = 180)));
  SolarTherm.Models.PowerBlocks.sCO2Cycle.DirectDesign.HeatRecuperatorDTAve Rec(N_q = 8, pinchRecuperator = 20)  annotation(
  ...
  SolarTherm.Models.PowerBlocks.sCO2Cycle.DirectDesign.HeatRecuperatorDTAve low(N_q = 8, pinchRecuperator = 40) annotation(
    Placement(visible = true, transformation(origin = {-17, -55}, extent = {{-17, -17}, {17, 17}}, rotation = 0)));
  sCO2_cycle.Turbine Turbi(PR = 2.72628, m_flow_des = 77.4, n_shaft = 3479.209144, p_in_des = 2.5e+7) annotation(
    Placement(visible = true, transformation(origin = {90, 14}, extent = {{-16, -16}, {16, 16}}, rotation = -90)));
  Modelica.Fluid.Sources.MassFlowSource_T massFlowSource_T(redeclare package Medium = MediumPB,m_flow = -85.8, nPorts = 1, use_m_flow_in = false) annotation(
    Placement(visible = true, transformation(origin = {-52, -78}, extent = {{-8, -8}, {8, 8}}, rotation = 0)));
  Modelica.Fluid.Sources.Boundary_pT boundary_pT(redeclare package Medium = MediumPB,T = 99 + 273.15, nPorts = 1, p = 25e6, use_T_in = false, use_p_in = false) annotation(
    Placement(visible = true, transformation(origin = {-83, -7}, extent = {{7, -7}, {-7, 7}}, rotation = 180)));
  sCO2_cycle.Valve valve(gamma = 0.221)  annotation(
  ...
  Modelica.Fluid.Sources.Boundary_pT boundary_pT1(redeclare package Medium = MediumPB,T = 260 + 273.15, nPorts = 1, p = 25e6, use_T_in = false, use_p_in = false) annotation(
    Placement(visible = true, transformation(origin = {3, 85}, extent = {{-7, -7}, {7, 7}}, rotation = 180)));
  Modelica.Fluid.Sources.MassFlowSource_T massFlowSource_T1(redeclare package Medium = MediumPB,m_flow = -18.9618, nPorts = 1, use_m_flow_in = false) annotation(
    Placement(visible = true, transformation(origin = {-60, 46}, extent = {{-8, -8}, {8, 8}}, rotation = 0)));
initial equation
...
low.m_turb_des=85.8;
low.h_in_comp_des=500E3;
low.p_in_comp_des=25e6;
low.m_comp_des=18.9618;
equation
  connect(Sourc.T_in, exponentials.y) annotation(
    Line(points = {{53, 68}, {60, 68}, {60, 70}, {67, 70}}, color = {0, 0, 127}));
  ...
  connect(Turbi.port_b, Rec.from_turb_port_a) annotation(
    Line(points = {{84, 4}, {84, -60}, {49, -60}}, color = {0, 127, 255}));
  connect(Turbi.port_a, Sourc.ports[1]) annotation(
    Line(points = {{93, 24}, {26, 24}, {26, 71}, {38, 71}}, color = {0, 127, 255}));
  connect(boundary.ports[1], Rec.from_comp_port_b) annotation(
    Line(points = {{56, -8}, {60, -8}, {60, -50}, {50, -50}}, color = {0, 127, 255}));
  connect(massFlowSource_T.ports[1], low.from_turb_port_b) annotation(
    Line(points = {{-44, -78}, {-30, -78}, {-30, -60}}, color = {0, 127, 255}));
  connect(boundary_pT.ports[1], low.from_comp_port_a) annotation(
    Line(points = {{-76, -6}, {-28, -6}, {-28, -50}, {-28, -50}}, color = {0, 127, 255}));
  connect(low.from_comp_port_b, flowmixer.first_port_a) annotation(
    Line(points = {{-4, -50}, {-4, -41}, {2, -41}, {2, -42}}, color = {0, 127, 255}));
  connect(valve.one_gamma_port_b, flowmixer.second_port_a) annotation(
    Line(points = {{0, 38}, {0, -36}, {10, -36}}, color = {0, 127, 255}));
  connect(flowmixer.port_b, Rec.from_comp_port_a) annotation(
    Line(points = {{18, -42}, {26, -42}, {26, -50}}, color = {0, 127, 255}));
  connect(boundary_pT1.ports[1], valve.port_a) annotation(
    Line(points = {{-4, 86}, {-12, 86}, {-12, 54}, {0, 54}, {0, 54}}, color = {0, 127, 255}));
  connect(massFlowSource_T1.ports[1], valve.gamma_port_b) annotation(
    Line(points = {{-52, 46}, {-8, 46}, {-8, 46}, {-8, 46}}, color = {0, 127, 255}));
  annotation(
