within sCO2_cycle;
model Exchanger "simple generic model"
		...
		parameter  SI.SpecificEnthalpy[N_exch] state_CO2_des;
		parameter  SI.SpecificEnthalpy[N_exch] state_HTF_des ;
		 SI.SpecificEnthalpy[N_exch] state_CO2;
		 SI.SpecificEnthalpy[N_exch]  state_HTF;
		...
		Modelica.Fluid.Interfaces.FluidPort_a HTF_port_a(redeclare package Medium =Medium) annotation(
			Placement(visible = true, transformation(origin = {70, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0),
			iconTransformation(origin = {70, 40}, extent = {{-6, -6}, {6, 6}}, rotation = 0)));
		Modelica.Fluid.Interfaces.FluidPort_a CO2_port_a(redeclare package Medium = Medium) annotation(
			Placement(visible = true, transformation(origin = {-70, -40}, extent = {{-10, -10}, {10, 10}}, rotation = 0),
			iconTransformation(origin = {-70, -40}, extent = {{-6, -6}, {6, 6}}, rotation = 0)));
		Modelica.Fluid.Interfaces.FluidPort_b HTF_port_b(redeclare package Medium = Medium) annotation(
			Placement(visible = true, transformation(origin = {-70, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0),
			iconTransformation(origin = {-70, 40}, extent = {{-6, -6}, {6, 6}}, rotation = 0)));
		Modelica.Fluid.Interfaces.FluidPort_b CO2_port_b(redeclare package Medium = Medium) annotation(
			Placement(visible = true, transformation(origin = {70, -40}, extent = {{-10, -10}, {10, 10}}, rotation = 0),
			iconTransformation(origin = {70, -40}, extent = {{-6, -6}, {6, 6}}, rotation = 0)));
	...
	initial equation
		...
		deltaT_des[i]=stprops("T","H",855004,"P",1e5,"R744")-stprops("T","H",1e6,"P", 25e6,"R744");
			state_CO2_des[i] =stprops("H","T",T_CO2_des[i],"P",p_in_CO2_des,"R744"); 
			state_HTF_des[i] = stprops("H","T",T_HTF_des[i],"P",p_in_HTF_des,"R744");
		end for;

		T_CO2_des[N_exch]=T_out_CO2_des;

		for i in 1:(N_exch-1) loop
			Q_HX_des = ratio_m_des * (state_CO2_des[i+1]- state_CO2_des[i]);
			Q_HX_des =	(state_HTF_des[i+1]- state_HTF_des[i]);
		...
		h_in_HTF_des = state_HTF_des[N_exch];
		h_out_HTF_des = state_HTF_des[1];
		h_in_CO2_des = state_CO2_des[1];
		h_out_CO2_des = state_CO2_des[N_exch];
	...
	for i in 1:N_exch loop
deltaT[i] = if m_sup then stprops("T","H",h_HTF[i],"P",HTF_port_a.p,"R744")-stprops("T","H",h_CO2[i] ,"P",CO2_port_a.p,"R744") else deltaT_des[i];
		state_CO2[i] =stprops("T","H",h_CO2[i],"P",CO2_port_a.p ,"R744");
		state_HTF[i] = stprops("T","H",h_HTF[i],"P",HTF_port_a.p,"R744");
	end for;

	T_CO2_out = state_CO2[N_exch]; // Cold outlet temperature at the last node
	T_HTF_out = state_HTF[1];     // Hot outlet temperature at the first node

	deltaTAve = (deltaT[1] + deltaT[N_exch]) / 2; // Setting an average temperature difference

	h_CO2[N_exch] = inStream( CO2_port_b.h_outflow);
