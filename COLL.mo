...
import Modelica.Math;
	...
	replaceable package Medium_A =sCO2_cycle.AIR "Hot stream medium";
...
parameter Integer N=8;
	...
	parameter SI.TemperatureDifference dT_approach =-5 "Minimum temperature difference between streams";
	...
	parameter SI.Pressure p_b_des = 8e5 "Cold stream pressure";
	parameter SI.Temperature T_a_in_des = from_degC(45) "Cold stream outlet temperature at design";
	parameter SI.Temperature T_a_out_des = from_degC(200) "Cold stream outlet temperature at design";
	parameter SI.Temperature T_b_in_des = from_degC(600) "Cold stream outlet temperature at design";
	parameter SI.Temperature T_b_out_des = from_degC(45) "Cold stream outlet temperature at design";
...
equation
	...
	port_a_out.m_flow + port_a_in.m_flow = 0;
if fixed_m_flow then
    port_b_out.m_flow = -m_flow;
  else
    port_b_in.m_flow + port_b_out.m_flow = 0; // Cold stream
  end if;
	...
	T_a_in = stprops("T","P",port_a_in.p,"H",inStream(port_a_in.h_outflow),"R729");
	T_b_in = Medium_B.temperature(state_b_in);


	dT_approach =  T_b_in-T_a_out ;

	Q_flow = (port_b_in.m_flow*( port_b_out.h_outflow-inStream(port_b_in.h_outflow)));

	LMTD = (((-T_b_out+T_a_in)-dT_approach)/(Math.log(((-T_b_out+T_a_in)/dT_approach))));
